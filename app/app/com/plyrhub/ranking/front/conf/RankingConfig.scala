...
object RankingConfig {
  ...
  object ModelConstraints {
    ...
    // Member
    def memberIdMinLength = DynamicIntProperty("model.member.id.min.length", 1).get

    def memberIdMaxLength = DynamicIntProperty("model.member.id.max.length", 20).get

    def memberRankingsMin = DynamicIntProperty("model.member.rankings.min", 1).get

    def memberRankingsMax = DynamicIntProperty("model.member.rankings.max", 100).get
  }
